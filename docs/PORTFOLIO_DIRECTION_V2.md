...
- **Content substance is preserved.** Thesis, testing data, and honest scope statements ("designed but not fully wired into the shipped build") carry over verbatim. Later editorial tightening for hierarchy/brevity/recruiter scanning is permitted (Approved amendment #3) — cutting substance for visual convenience is not.
- **Ownership lines are preserved verbatim.** FrankenTeen's credit grid (teammates named, Act 3 contribution isolated) and BETTR/Echoes' self-written-vs-adapted statements carry over exactly as written — this pattern is not to be summarized, softened, or removed for layout reasons.
